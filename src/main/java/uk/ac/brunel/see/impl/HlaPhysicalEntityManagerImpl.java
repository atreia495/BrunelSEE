...
final class HlaPhysicalEntityManagerImpl extends AbstractObjectClassManager implements HlaPhysicalEntityManager {
   ...
   private final Map<ObjectInstanceHandle, HlaPhysicalEntityImpl> _remoteInstances =
         new ConcurrentHashMap<ObjectInstanceHandle, HlaPhysicalEntityImpl>();
   private final CacheMap<String, HlaPhysicalEntityImpl> _getByNameCache =
   ...
   public HlaPhysicalEntityImpl getPhysicalEntityByName(String name) {
      HlaPhysicalEntityImpl cachedInstance = _getByNameCache.get(name);
      ...
      for (HlaPhysicalEntityImpl instance : MapAdapter.concat(_localInstances.values(), _remoteInstances.values())) {
         if (instance.hasName() &&
   ...
   private void requestValueUpdates(final HlaPhysicalEntityImpl instance, final ObjectInstanceHandle objectInstanceHandle, final Set<String> requestAttributes, int retries) {
   ...
   private void repeatRequestIfNotInitialized(final HlaPhysicalEntityImpl instance, final ObjectInstanceHandle objectInstanceHandle, final Set<String> requestAttributes, int retries) {
      try {
                      ...
                      if (!instance.isInitialized() && instance.isWithinInterest()) {
                         requestValueUpdates(
                               ...
                               instance.getUnsetAttributeNames()
                                     .stream()
                                     .filter(HlaPhysicalEntityImpl.HLA_INITIALIZED_ATTRIBUTE_NAMES::contains)
